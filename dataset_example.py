"""Script usage:To show the 0th training image with its label data
...
def get_image(n,is_test):
    ...
    subject = Image.open((get_imlist(is_test))[n]).convert("L")
...
#    mask=Image.open(str(get_imlist(n)+'mask.tif').convert("RGB")
    ...
    mask=Image.open(REAL_PATH_TRAIN+'/'+os.path.basename((
        get_imlist(False)[n])).split('.')[0]+'_mask.tif').convert("L")
